...
public static class VaultService
{
    ...
    private static string Root => Path.Combine(LinuxPaths.DataRoot, "vault");
    ...
    public static VaultItem Import(string source)
    {
        ...
        SettingsStore.TryPrivatePermissions(finalPath);
        return item;
    }
    ...
    public static void Delete(VaultItem item) { var path = ItemPath(item.Id); if (File.Exists(path)) File.Delete(path); }
    public static string ItemPath(string id) => Path.Combine(Root, id + ".hxd");

    public static void RunSelfTest()
    {
        var root = Path.Combine(Path.GetTempPath(), "HexaDockLinux-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        try
        {
    ...
    private static byte[] GetKey()
    {
        ...
        SettingsStore.TryPrivatePermissions(KeyPath);
        return key;
    }
    ...
    private static void Encrypt(string source, string destination, byte[] key)
    {
        ...
        while ((count = input.Read(buffer, 0, buffer.Length)) > 0)
        {
            ...
            writer.Write(count); writer.Write(nonce); writer.Write(tag); writer.Write(cipher);
        }
        writer.Write(0);
    }
    ...
    private static void Decrypt(string source, string destination, byte[] key)
    {
        ...
        while (true)
        {
            ...
            var nonce = reader.ReadBytes(12); var tag = reader.ReadBytes(16); var cipher = reader.ReadBytes(count);
